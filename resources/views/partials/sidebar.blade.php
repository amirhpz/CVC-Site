    ...
    <ul class="menu-inner py-1">
        ...
        @foreach($menupanels as $menupanel)
            ...
            <li class="menu-item {{ $isActive }}">
                @php
                    $menuUrl = $menuRouteMap[$menupanel->slug] ?? url('panel/' . ltrim($menupanel->slug, '/'));
                @endphp
                <a class="menu-link d-flex align-items-center {{ $isToggle ? 'menu-toggle' : '' }}"
                   href="{{ $isToggle ? 'javascript:void(0);' : $menuUrl }}">
                    ...
                    <ul class="menu-sub px-2">
                        @foreach($menupanel->accessible_submenus as $submenu)
                            @if(Gate::allows('can-access', [$submenu->slug, 'view']))
                                @php
                                    $submenuUrl = $submenuRouteMap[$submenu->slug] ?? (
                                        Route::has($submenu->slug . '.index')
                                            ? route($submenu->slug . '.index')
                                            : url('panel/' . $submenu->slug)
                                    );
                                @endphp
